...
export default function CatchUpScreen() {
  ...
  const fetchMentions = async () => {
    try {
      const data = await api.messages.getMentions();
      setMessages(data.mentions || []);
    } catch (e) {
  ...
  return (
    <View style={[styles.container, { paddingTop: insets.top }]}>
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>{t('quick.catch_up_title', 'Catch Up')}</Text>
        <View style={{ width: 40 }} />
      </View>
      ...
      {loading ? (
        ...
        <View style={styles.center}>
          <MaterialIcons name="auto-awesome" size={48} color={colors.border} />
          <Text style={styles.emptyText}>{t('quick.all_caught_up')}</Text>
          <Text style={styles.emptySub}>{t('quick.no_recent_mentions')}</Text>
        </View>
      ) : (
        <FlatList
          ...
          renderItem={({ item }) => (
            <MessageCard
              message={item}
              onPress={() => router.push(`/chat/${item.channel_id}`)}
            />
          )}
