  ...
  return (
    <aside className={`sidebar ${isOpen ? 'open' : ''}`}>
      ...
      <div className="sidebar-scroll">
        {(activeView === 'chat' || activeView === 'threads' || activeView === 'saved' || activeView === 'search') && (
          <>
              ...
              <div id="sec-announcements">
                {announcementChannels.map(ch => (
                  <div key={ch.id} className={`nav-item ${!isAdminActive && activeView === 'chat' && activeChannel === ch.slug ? 'active' : ''}`} onClick={() => setActiveChannel(ch.slug)}>
                    <span className="icon"><ChannelIcon type="announcement" name={ch.name} icon={ch.icon} color={ch.color} size={14} /></span>
            ...
            {sections.channels && (
              <div id="sec-channels">
                {publicPrivateChannels.map(ch => (
                  <div key={ch.id} className={`nav-item ${!isAdminActive && activeView === 'chat' && activeChannel === ch.slug ? 'active' : ''}`} onClick={() => setActiveChannel(ch.slug)}>
                    <span className="icon"><ChannelIcon type={ch.type} name={ch.name} icon={ch.icon} color={ch.color} size={14} /></span>
                    <span className="name">{ch.name}</span>
                    <UnreadBadge channelId={ch.id} />
                  </div>
                ))}
